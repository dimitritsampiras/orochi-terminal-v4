...
type Session = Pick<typeof batches.$inferSelect, "id" | "active" | "createdAt"> & {
    ...
    <>
      <div className="@container/table bg-white rounded-lg shadow-sm border border-zinc-200 w-full">
        <Table className="w-full">
          ...
          <TableBody>
            {sessions.length > 0 ? (
              sessions.map(({ id, createdAt, orders, active }) => (
                <TableRow key={id} onClick={() => handleRowClick(id)} className="cursor-pointer hover:bg-zinc-50">
                  ...
                  <TableCell className="font-semibold">{orders.length}</TableCell>
                  <TableCell>
                    <Progress
                      color="emerald"
                      value={
                        (orders.filter((order) => order.displayFulfillmentStatus === "FULFILLED").length /
                          orders.length) *
                        100
                      }
                    />
              ...
              <TableRow>
                <TableCell colSpan={8} className="text-center">
                  <p className="text-sm text-muted-foreground py-4">No batches found</p>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
