...
import type { ShipmentIssueType } from "@/lib/core/shipping/shipping-utils";
...
function getShipmentIssue(
	...
	const hasUnpurchased = shipmentsList.some(
		(s) => !s.isPurchased && !s.isRefunded,
	);
	if (hasUnpurchased) {
	...
	const hasMissingLabel = shipmentsList.some(
		(s) => s.isPurchased && !s.isRefunded && !s.labelSlipPath,
	);
	if (hasMissingLabel) {
