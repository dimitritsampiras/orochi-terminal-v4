	...
	return (
		<div>
			...
			<div className="my-4 flex flex-col gap-2">
					...
					<Alert className="text-amber-700 bg-amber-50">
						...
						<AlertDescription>
							<p>
								...
								levels are adjusted. You can print duplicate items{" "}
								<Link
				...
				{item.productVariant?.warehouseInventory != null &&
					item.productVariant.warehouseInventory > 0 && (
						<Alert className="text-blue-800 bg-blue-50">
							<Icon icon="ph:warning-circle" className="size-4" />
							<AlertTitle>Product is in stock</AlertTitle>
							<AlertDescription>
								<p>
									This product is in stock. Please use warehouse inventory. If
									this item is not in stock, please adjust the warehouse
									inventory.
								</p>
							</AlertDescription>
						</Alert>
					...
					<TooltipProvider>
						<Tooltip>
							...
							{batchId && !hasProductStock && (
								<TooltipContent side="top">
									Product stock is empty - cannot mark as in stock
								</TooltipContent>
							)}
						</Tooltip>
					</TooltipProvider>
