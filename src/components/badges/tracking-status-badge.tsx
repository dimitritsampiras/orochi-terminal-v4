...
export const EasyPostTrackingStatusBadge = ({ status }: { status: EasypostShipmentStatus }) => {
  const colorMap: Record<EasypostShipmentStatus, string> = {
    delivered: "bg-green-100 text-green-800",
    failure: "bg-red-100 text-red-800",
    in_transit: "bg-blue-100 text-blue-800",
    out_for_delivery: "bg-yellow-100 text-yellow-800",
    unknown: "bg-gray-200 text-gray-700",
    available_for_pickup: "bg-purple-100 text-purple-800",
    return_to_sender: "bg-red-100 text-red-800",
    cancelled: "bg-yellow-200 text-yellow-800",
    error: "bg-red-100 text-red-800",
    pre_transit: "bg-gray-200 text-gray-700",
  };
  return (
    <Badge variant="secondary" className={cn(colorMap[status || "UNKNOWN"], "uppcase text-[10px]")}>
      {status?.replace("_", " ") || "UNKNOWN"}
