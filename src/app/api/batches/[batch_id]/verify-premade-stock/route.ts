...
export async function GET(
	...
	try {
		...
		if (error || !data) {
			return NextResponse.json(
				{ data: null, error: error ?? "Failed to fetch line items" },
				{ status: 500 },
			);
		}
		...
		const { items, malformedItems } = getPremadeStockRequirements(lineItems);
		...
		if (productVariantIds.length === 0) {
			return NextResponse.json(
				{ data: null, error: "No products found" },
				{ status: 400 },
			);
		}
		...
		const inventoryMap = new Map(
			currentInventories.map((pv) => [pv.id, pv.warehouseInventory]),
		);
		...
		const itemWithInventory: PremadeStockItemWithInventory[] = items.map((stockItem) => ({
			...
			inventoryTransactions: currentTransactions.filter(
				(t) => t.productVariantId === stockItem.productVariantId,
			),
		}));
		...
		return NextResponse.json({
			data: {
				...
				malformedItems,
				filteredItems: filteredLineItems,
			},
			error: null,
		});
	} catch (error) {
		...
		return NextResponse.json(
			{
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to fetch premade stock requirements",
			},
			{ status: 500 },
		);
	}
