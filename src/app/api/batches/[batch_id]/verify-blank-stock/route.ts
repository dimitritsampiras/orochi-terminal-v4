...
export async function GET(
	...
	if (!batch) {
		return NextResponse.json(
			{ data: null, error: "Cannot find session" },
			{ status: 400 },
		);
	}
	...
	try {
		...
		if (blankVariantIds.length === 0) {
			return NextResponse.json(
				{ data: null, error: "No blank variants found" },
				{ status: 400 },
			);
		}
		...
		const itemsWithTransactions: BlankStockItemWithInventory[] = items.map(
			(blankItem) => ({
				...blankItem,
				inventoryTransactions: currentTransactions.filter(
					(t) => t.blankVariantId === blankItem.blankVariantId,
				),
			}),
		);
		...
		return NextResponse.json(
			{
				...
				error:
					error instanceof Error
						? error.message
						: "Failed to fetch blank stock requirements",
			},
			{ status: 500 },
		);
	}
