...
export type FulfillmentOrdersQueryVariables = AdminTypes.Exact<{
  id: AdminTypes.Scalars['ID']['input'];
}>;


export type FulfillmentOrdersQuery = { node?: AdminTypes.Maybe<(
    ...
    { __typename: 'Order' }
    & Pick<AdminTypes.Order, 'name' | 'createdAt' | 'updatedAt' | 'cancelledAt' | 'id'>
    & { fulfillmentOrders: { nodes: Array<(
        Pick<AdminTypes.FulfillmentOrder, 'channelId' | 'createdAt' | 'fulfillAt' | 'id' | 'status' | 'orderId' | 'requestStatus'>
        & { assignedLocation: Pick<AdminTypes.FulfillmentOrderAssignedLocation, 'name' | 'address1'>, destination?: AdminTypes.Maybe<Pick<AdminTypes.FulfillmentOrderDestination, 'address1' | 'countryCode'>>, lineItems: { nodes: Array<Pick<AdminTypes.FulfillmentOrderLineItem, 'id' | 'productTitle'>> } }
      )> } }
  ) | (
...
export type CreateFulfillmentMutation = { fulfillmentCreateV2?: AdminTypes.Maybe<{ fulfillment?: AdminTypes.Maybe<Pick<AdminTypes.Fulfillment, 'id' | 'status'>>, userErrors: Array<Pick<AdminTypes.UserError, 'field' | 'message'>> }> };
...
export type OrderQuery = { node?: AdminTypes.Maybe<(
    ...
    & { totalPriceSet: { shopMoney: Pick<AdminTypes.MoneyV2, 'amount'> }, risk: { assessments: Array<Pick<AdminTypes.OrderRiskAssessment, 'riskLevel'>> }, localizedFields: { nodes: Array<Pick<AdminTypes.LocalizedField, 'key' | 'purpose' | 'value'>> }, customer?: AdminTypes.Maybe<Pick<AdminTypes.Customer, 'firstName' | 'lastName' | 'numberOfOrders' | 'email' | 'tags'>>, app?: AdminTypes.Maybe<Pick<AdminTypes.OrderApp, 'name'>>, shippingLine?: AdminTypes.Maybe<Pick<AdminTypes.ShippingLine, 'title'>>, shippingAddress?: AdminTypes.Maybe<Pick<AdminTypes.MailingAddress, 'address1' | 'address2' | 'city' | 'country' | 'countryCodeV2' | 'firstName' | 'lastName' | 'phone' | 'company' | 'provinceCode' | 'province' | 'zip'>>, lineItems: { nodes: Array<(
        ...
        & { image?: AdminTypes.Maybe<Pick<AdminTypes.Image, 'id' | 'url'>>, originalTotalSet: { shopMoney: Pick<AdminTypes.MoneyV2, 'amount'> }, variant?: AdminTypes.Maybe<(
          Pick<AdminTypes.ProductVariant, 'id'>
...
export type OrdersQuery = { orders: { pageInfo: Pick<AdminTypes.PageInfo, 'hasNextPage' | 'hasPreviousPage' | 'startCursor' | 'endCursor'>, nodes: Array<(
      Pick<AdminTypes.Order, 'id' | 'name' | 'createdAt' | 'updatedAt' | 'cancelledAt' | 'note' | 'displayFinancialStatus' | 'discountCodes' | 'displayFulfillmentStatus'>
      & { totalPriceSet: { shopMoney: Pick<AdminTypes.MoneyV2, 'amount'> }, risk: { assessments: Array<Pick<AdminTypes.OrderRiskAssessment, 'riskLevel'>> }, app?: AdminTypes.Maybe<Pick<AdminTypes.OrderApp, 'name'>>, shippingLine?: AdminTypes.Maybe<Pick<AdminTypes.ShippingLine, 'title'>>, lineItems: { nodes: Array<(
          Pick<AdminTypes.LineItem, 'id' | 'title' | 'variantTitle' | 'quantity' | 'name' | 'vendor' | 'unfulfilledQuantity' | 'requiresShipping' | 'nonFulfillableQuantity'>
          & { image?: AdminTypes.Maybe<Pick<AdminTypes.Image, 'id' | 'url'>>, originalTotalSet: { shopMoney: Pick<AdminTypes.MoneyV2, 'amount'> }, variant?: AdminTypes.Maybe<(
            Pick<AdminTypes.ProductVariant, 'id'>
            & { inventoryItem: { measurement: { weight?: AdminTypes.Maybe<Pick<AdminTypes.Weight, 'unit' | 'value'>> } } }
...
export type ProductMediaQueryQuery = { files: { nodes: Array<Pick<AdminTypes.ExternalVideo, 'createdAt' | 'updatedAt' | 'alt'> | Pick<AdminTypes.GenericFile, 'createdAt' | 'updatedAt' | 'alt'> | (
      { __typename: 'MediaImage' }
