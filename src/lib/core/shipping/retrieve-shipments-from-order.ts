...
import type { shipments } from "@drizzle/schema";
import type { Transaction, Shipment as ShippoShipment } from "shippo";
			...
			return shippo.shipments
				...
				.then(async (shippoShipment) => {
					...
					if (orderShipment.shippoTransactionId) {
						transaction = await shippo.transactions
							.get(orderShipment.shippoTransactionId)
							.catch(() => null);
						if (transaction === null) {
							issues.push(
								`Transaction ${orderShipment.shippoTransactionId} not found in Shippo`,
							);
						}
					}
					...
					const shippoData: ShippoShipmentData = {
						...
						shippoInfo: {
							...shippoShipment,
							chosenRate:
								shippoShipment.rates.find(
									(rate) => rate.objectId === orderShipment.chosenRateId,
								) || null,
							transaction,
							issues,
						},
				...
				.catch((err) => {
					logger.error(
						`Failed to retrieve Shippo shipment ${orderShipment.shipmentId}:`,
						err,
					);
					return null;
				});
		...
		.map(async (orderShipment): Promise<EasyPostShipmentData | null> => {
			return easypost.Shipment.retrieve(orderShipment.shipmentId)
				.then(async (easypostShipment) => {
					...
					const plainShipment = JSON.parse(
						JSON.stringify(easypostShipment),
					) as typeof easypostShipment;
					const easypostData: EasyPostShipmentData = {
						...orderShipment,
						api: "EASYPOST",
						easypostInfo: {
							chosenRate:
								plainShipment.rates.find(
									(rate) => rate.id === orderShipment.chosenRateId,
								) || null,
							issues: [],
							...plainShipment,
						},
				...
				.catch((err) => {
					logger.error(
						`Failed to retrieve EasyPost shipment ${orderShipment.shipmentId}: ${err.message}`,
					);
					return null;
				});
		});

	const orderShipmentWithData = await Promise.all([
		...orderShipmentWithShippoInfo,
		...orderShipmentWithEasypostInfo,
	]);
