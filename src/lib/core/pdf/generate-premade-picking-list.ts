...
import { getPremadeStockRequirements, PremadeStockItem } from "../session/get-premade-stock-requirements";
...
export const generatePremadePickingList = async (
  ...
  const { items, unaccountedItems, heldItems } = getPremadeStockRequirements(sessionLineItems);
  ...
  if (stockItems.length > 0) {
    ...
    page.drawText("Pre-printed overstock and black label items to pick from shelves.", {
      x: marginX,
  ...
  if (unaccountedItems.length > 0) {
    ...
    page.drawText(
      "These items have missing product or blank data and cannot be processed. Either ignore or contact admin to fix product setup.",
      {
        x: marginX,
        y: toY(currentY + smallFontSize),
        size: smallFontSize,
