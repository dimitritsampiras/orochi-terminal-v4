...
import { SessionLineItem } from "../session/get-session-line-items";
import { getBlankStockRequirements, BlankStockItem } from "../session/get-blank-stock-requirements";
import { PremadeStockItem } from "../session/get-premade-stock-requirements";
...
export const generateBlankPickingList = async (
  ...
  const { items, unaccountedItems, heldItems } = getBlankStockRequirements(sessionLineItems, premadeStockItems);
  ...
  const blankHeaders = ["Blank", "Color / Size", "Type", "On Hand", "Req.", "To Pick", "Check"];
  ...
  if (blankItems.length > 0) {
    ...
    page.drawText("Blank garments needed for printing. Items not covered by premade stock.", {
      x: marginX,
  ...
  if (heldItems.length > 0) {
    ...
    page.drawText("Orders on Hold — (ignore)", {
      x: marginX,
      y: toY(currentY + headerFontSize),
      size: headerFontSize,
      font: fontBold,
    ...
    page.drawText("These items are from orders currently on hold. Do not pick, they are excluded from this session.", {
      x: marginX,
      y: toY(currentY + smallFontSize),
      size: smallFontSize,
      font: fontItalic,
    ...
    for (const item of heldItems) {
      ...
      page.drawText(`(${item.orderNumber})`, {
        x: marginX + 300,
        y: toY(currentY + bodyFontSize),
        size: smallFontSize,
        font,
