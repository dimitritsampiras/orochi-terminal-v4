...
/**
 * Represents an order with its line items (minimal info for counts/status)
 */
...
export const getLineItemsByBatchId = async (
  batchId: number
): Promise<DataResponse<{ lineItems: SessionLineItem[]; batch: typeof batches.$inferSelect }>> => {
  try {
    ...
    const ordersWithActiveHolds = new Set(activeHolds.map((h) => h.orderId).filter(Boolean) as string[]);
 ...
 */
export const getLineItemById = async (lineItemId: string): Promise<DataResponse<SessionLineItemWithPrintLogs>> => {
  try {
    const results = await db
      .select({
        lineItem: lineItems,
