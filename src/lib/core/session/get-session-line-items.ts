...
import { FulfillmentType } from "./create-picking-requirements";
import { AssemblyLineItemWithPrintLogs } from "./create-assembly-line";
...
export const getLineItemById = async (
  lineItemId: string,
  batchId?: number,
): Promise<DataResponse<AssemblyLineItemWithPrintLogs>> => {
  try {
    const results = await db
      .select({
        lineItem: lineItems,
    ...
    let expectedFulfillment: FulfillmentType = "print"; // default
    let itemPosition: number = -1;

    if (batchId) {
      const batch = await db.query.batches.findFirst({
        where: { id: batchId },
        columns: { assemblyLineJson: true },
      });

      if (batch?.assemblyLineJson) {
        const stored = JSON.parse(batch.assemblyLineJson);
        const found = stored.find((item: { id: string }) => item.id === lineItemId);
        if (found?.expectedFulfillment) {
          expectedFulfillment = found.expectedFulfillment;
          itemPosition = found.itemPosition;
        }
      }
    }
    ...
    const sessionLineItem: AssemblyLineItemWithPrintLogs = {
      ...firstRow.lineItem,
      order: {
        ...firstRow.order,
