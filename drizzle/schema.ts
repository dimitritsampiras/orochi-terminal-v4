import { pgEnum, pgTable, integer, bigserial, text, uuid, bigint, timestamp, boolean, numeric, jsonb, doublePrecision, json, index, uniqueIndex, foreignKey, primaryKey, unique, pgPolicy } from "drizzle-orm/pg-core"
...
export const expenseCategory = pgEnum("expense_category", ["rent", "salary", "marketing_meta", "marketing_google", "sponsorship", "other", "software_saas", "advertising", "shipping_logistics", "labor_payroll", "inventory_blanks", "subscriptions", "banking_fees", "insurance", "refunds_chargebacks", "internal_transfer", "personal", "inventory", "dtg_printing", "software", "chargeback", "commissions", "influencers", "shipping", "contractors", "utilities", "credit_card_payments", "shopify_payouts", "fees", "refunds", "blanks_inventory", "dtf_print_services", "international_payments", "software_subscriptions", "shipping_postage", "marketing", "premade_garments", "sales", "warehouse"])


export const batchDocuments = pgTable.withRLS("batch_documents", {
...
export const blankVariants = pgTable.withRLS("blank_variants", {
...
export const csvTransactions = pgTable.withRLS("csv_transactions", {
	id: uuid().defaultRandom().primaryKey(),
	createdAt: timestamp("created_at", { withTimezone: true }).default(sql`now()`).notNull(),
	...
	lastModifiedAt: timestamp("last_modified_at", { withTimezone: true }).default(sql`now()`).notNull(),
	currency: text().default("USD").notNull(),
}, (table) => [
	index("csv_transactions_period_idx").using("btree", table.periodYear.asc().nullsLast(), table.periodMonth.asc().nullsLast()),
	index("csv_transactions_reconciliation_idx").using("btree", table.reconciliationGroupId.asc().nullsLast()),
	index("csv_transactions_source_idx").using("btree", table.source.asc().nullsLast()),
	index("csv_transactions_vendor_idx").using("btree", table.vendor.asc().nullsLast()),
	unique("csv_transactions_unique_source_transaction").on(table.source, table.sourceTransactionId, table.periodYear, table.periodMonth),]);

export const globalSettings = pgTable.withRLS("global_settings", {
	...
	inkCostPerItem: doublePrecision("ink_cost_per_item").default(1.2).notNull(),
	...
	costBufferPercentage: doublePrecision("cost_buffer_percentage").default(10).notNull(),
});
...
export const lineItems = pgTable.withRLS("line_items", {
	id: text().primaryKey(),
...
export const logs = pgTable.withRLS("logs", {
	id: bigint({ mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
	createdAt: timestamp("created_at", { withTimezone: true }).default(sql`now()`).notNull(),
...
export const monthlyPeriods = pgTable.withRLS("monthly_periods", {
	id: uuid().defaultRandom().primaryKey(),
	periodMonth: integer("period_month").notNull(),
	periodYear: integer("period_year").notNull(),
	...
	totalRevenue: numeric("total_revenue", { mode: 'number', precision: 12, scale: 2 }).default(0).notNull(),
	totalExpenses: numeric("total_expenses", { mode: 'number', precision: 12, scale: 2 }).default(0).notNull(),
	netCashFlow: numeric("net_cash_flow", { mode: 'number', precision: 12, scale: 2 }).default(0).notNull(),
...
export const orderHolds = pgTable.withRLS("order_holds", {
	id: bigint({ mode: 'number' }).primaryKey().generatedByDefaultAsIdentity(),
	createdAt: timestamp("created_at", { withTimezone: true }).default(sql`now()`).notNull(),
...
export const orderNotes = pgTable.withRLS("order_notes", {
	id: uuid().defaultRandom().primaryKey(),
	createdAt: timestamp("created_at", { withTimezone: true }).default(sql`now()`).notNull(),
	orderId: text("order_id").notNull().references(() => orders.id, { onDelete: "cascade" } ),
...
export const ordersBatches = pgTable.withRLS("orders_batches", {
	orderId: text("order_id").notNull().references(() => orders.id),
...
export const parcelTemplates = pgTable.withRLS("parcel_templates", {
...
export const printLogs = pgTable.withRLS("print_logs", {
	id: uuid().defaultRandom().primaryKey(),
...
export const prints = pgTable.withRLS("prints", {
	id: uuid().defaultRandom().primaryKey(),
...
export const recurringExpenses = pgTable.withRLS("recurring_expenses", {
	id: uuid().defaultRandom().primaryKey(),
	createdAt: timestamp("created_at", { withTimezone: true }).default(sql`now()`).notNull(),
	name: text().notNull(),
...
export const weeklyReports = pgTable("weekly_reports", {
	...
	refunds: numeric({ mode: 'number', precision: 12, scale: 2 }).default(0).notNull(),
	returns: numeric({ mode: 'number', precision: 12, scale: 2 }).default(0).notNull(),
	...
	payrollSource: text("payroll_source"),
	marketingCostMeta: numeric("marketing_cost_meta", { mode: 'number', precision: 12, scale: 2 }).default(0),
	marketingCostGoogle: numeric("marketing_cost_google", { mode: 'number', precision: 12, scale: 2 }).default(0),
	marketingCostOther: numeric("marketing_cost_other", { mode: 'number', precision: 12, scale: 2 }).default(0),
	totalMarketingCost: numeric("total_marketing_cost", { mode: 'number', precision: 12, scale: 2 }).default(0).notNull(),
	recurringExpenses: numeric("recurring_expenses", { mode: 'number', precision: 12, scale: 2 }).default(0).notNull(),
	csvExpenses: numeric("csv_expenses", { mode: 'number', precision: 12, scale: 2 }).default(0),
	csvExpenseSource: text("csv_expense_source"),
	...
	index("weekly_reports_week_start_idx").using("btree", table.weekStart.asc().nullsLast()),
	index("weekly_reports_year_idx").using("btree", table.year.asc().nullsLast()),
