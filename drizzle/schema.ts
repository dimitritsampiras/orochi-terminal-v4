...
export const blankVariants = pgTable(
  ...
  (table) => [unique("blank_variants_color_size_blank_id_key").on(table.color, table.size, table.blankId)]
);
...
export const ordersBatches = pgTable(
  ...
  (table) => [primaryKey({ columns: [table.orderId, table.batchId], name: "orders_session_batches_pkey" })]
);
